...
eq9 = I_ll*ddtheta_ll == (Fwhl*l_wl + Fbhl*l_bl)*sin(theta_ll) - (Fwsl*l_wl + Fbsl*l_bl)*cos(theta_ll)-T_lwl+T_bll;
eq10 = I_lr*ddtheta_lr == (Fwhr*l_wr + Fbhr*l_br)*sin(theta_lr) - (Fwsr*l_wr + Fbsr*l_br)*cos(theta_lr)-T_lwr+T_blr;
...
%     m_l , 0.168; %腿的质量
%     m_b , 4.56; %机体质量
%     I_w , 566.352*1.3/1000000; %轮的转动惯量
%     I_z , 0.03;
%     g, 9.8;
%     };
%%变量：R_l
...
sym_2_real = {
    R_w, 1; %轮半径
    R_l, 1; %机体半径
    l_wl, 1; %驱动轮质心到左右腿质心距离
    l_wr, 1; %默认为腿长一半
    l_bl, 1; %髋关节中点到腿质心距离
    l_br, 1; %默认为腿长一半
    I_ll, 1; %腿的转动惯量，轴心为髋关节中点
    I_lr, 1;
    I_b , 1;
    l_c , 1; %身体质心到髋关节中点连线距离
    m_w , 1; %轮的质量
    m_l , 1; %腿的质量
    m_b , 1; %机体质量
    I_w , 1; %轮的转动惯量
...
J_A_data = jacobian(formulas,jacobian_x)
J_B_data = jacobian(formulas,jacobian_u)
